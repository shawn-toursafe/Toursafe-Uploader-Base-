...
class UserService
{
	...
	public function __construct(){ 
		$this->product_id = 'C1222261';
		$this->api_url = 'https://license.berkine.space/';
		$this->api_key = '2F27259486BA258EC7BA';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["98031d069bf4d7e"])){
				$_SESSION["98031d069bf4d7e"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["98031d069bf4d7e"])){
				$get_data = $this->call_api(
					'POST',
