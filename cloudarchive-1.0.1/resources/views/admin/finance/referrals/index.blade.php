		...
		<div class="col-xl-3 col-lg-6 col-md-6 col-xm-12">
			<div class="card border-0">
				<div class="card-body">
					...
					<p class=" mb-3 fs-12 font-weight-bold mt-1">{{ __('Total Referral Allocated Storage Space') }} <span class="text-muted">({{ __('All Time') }})</span></p>
					<h2 class="mb-0"><span class="number-font-chars">{{ number_format($total_storage[0]['data']) }}</span></h2>
				</div>
			</div>
		</div>
	</div>
							...
							<div class="card-body">
								...
								<div class="row">
									<div class="col-lg-12 col-md-12 col-sm-12">								
										<div class="input-box">								
											<h6>{{ __('Allocate Storage Space') }} <span class="text-muted">({{ __('Only for Email Verified New Users') }})</span></h6>
											<div class="form-group">							    
												<input type="number" class="form-control @error('storage') is-danger @enderror" id="storage" name="storage" value="{{ config('payment.referral.registration.storage') }}">
												<span class="text-muted fs-10">{{ __('Size is in MB. You have to type only number e.g. value 100 means, user will have +100MB of storage space for each referred user.') }}</span>
											</div> 
											@error('storage')
